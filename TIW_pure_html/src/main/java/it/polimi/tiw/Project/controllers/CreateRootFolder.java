...
@WebServlet("/createRootFolder")
public class CreateRootFolder extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            //controllo che non esistano altre root folders con lo stesso nome
            try { 
    			FolderDAO folderDAO = new FolderDAO(connection);
    			if(false == folderDAO.uniqueRoot(ownerId ,foldername)) {
    				String errorMsg = "A root folder with the same name already exists";
    				response.sendRedirect("GoToContentManagement?errorMsgRF=" + URLEncoder.encode(errorMsg, "UTF-8"));
    			} else {
