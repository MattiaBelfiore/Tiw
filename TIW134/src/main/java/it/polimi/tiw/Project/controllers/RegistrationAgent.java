...
public class RegistrationAgent extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ripeti_pw = request.getParameter("ripeti_pw");
			...
			if(username.length()>15 || pw.length()>15) {
				isBadRequest = true;
		...
		*/
		if(isBadRequest) { /*C'� qualche errore nella compilazione della form */
			...
			if(userEsistente) {
				ctx.setVariable("errorReg", "Username gi� esistente.");
			}
			else if(lunghezzaCampiEsagerata) {
				ctx.setVariable("errorReg", "Lunghezza massima per i campi Username e Password di 15 caratteri.");
			}
			else if(emailScorretta) {
				ctx.setVariable("errorReg", "Email sintatticamente sbagliata");
			}
			else {
				ctx.setVariable("errorReg", "Le due password non corrispondono.");
			}
		...
		{	
			...
			String path;
			try {
					userDAO.createUser(username, name, surname, pw);
